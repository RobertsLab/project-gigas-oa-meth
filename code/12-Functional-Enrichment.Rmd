...
length(unique(sigRes.allBPAnnot$geneID)) #7 unique genes
length(unique(sigRes.allBPAnnot$transcript)) #34 unique transcripts
...
```
```{r}
sigRes.allBPProduct <- merge(sigRes.allBPAnnot, geneProductIDs, by = "transcript") #Add product annotations
...
```{r}
sigRes.allBPCluster <- merge(sigRes.allBPProduct, dfBP, by = "GOID") #Add cluster information to product annotations
...
```{r}
write.csv(sigRes.allBPCluster, "all-CC-EnrichedGO-DML-Cluster.csv", row.names = FALSE, quote = FALSE) #Save
```
...
sort(table(dfCC$cluster)) #Obtain size of clusters
```
