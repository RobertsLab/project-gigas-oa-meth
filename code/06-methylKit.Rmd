...
```{r}
nFiles <- 6 #Count number of samples
fileName <- data.frame("nameBase" = rep("../output/06-methylKit/general-stats/percent-CpG-methylation", times = nFiles),
                       ...
                       "sample.ID" = c(1, 3, 4, 6:8)) #Create new dataframe for filenames. Make sure ID matches with samples
...
```{r}
covariateMetadata <- data.frame("stage" = c("2", "1", "1",
...
```{r}
differentialMethylationStatsTreatment <- methylKit::calculateDiffMeth(methylationInformationFilteredCov5, covariates = covariateMetadata, overdispersion = "MN", test = "Chisq") #Calculate differential methylation statistics and include covariate information.
head(differentialMethylationStatsTreatment) #Look at differential methylation statistics
```
