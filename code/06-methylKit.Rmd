...
CGmotifsCounts <-as.data.frame(table(CGmotifs$V1)) #Count the number of genes/chromosome
...
```{r}
DMLchrCounts <-as.data.frame(table(diffMethStatsTreatment75Fem$chr)) #Count the number of DML/chromosome
colnames(DMLchrCounts) <- c("chr", "DMLCount") #Rename columns
head(DMLchrCounts) #Confirm formatting
```

```{r}
DMLchrCounts <- merge(x = DMLchrCounts, y = genechrCounts, by = "chr") #Merge by chr to get gene counts for each chr with DML
head(DMLchrCounts)
```
