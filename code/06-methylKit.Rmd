...
```{r}
pHRandTestResults25 <- t.test(pHRandTest25, alternative = "less", mu = nrow(diffMethStatsTreatment25)) #Conduct a one-sample t-test to determine the probability of identifying more DML by chance
pHRandTestResults25$statistic #t
pHRandTestResults25$parameter #df
pHRandTestResults25$p.value #P-value
```

```{r}
jpeg(filename = "../output/06-methylKit/rand-test/Random-Distribution-diff25.jpeg", height = 1000, width = 1000) #Save file with designated name
hist(pHRandTest25, plot = TRUE, main = "", xlab = "Number of DML (25% difference)") #Plot a histogram with randomization trial results
dev.off()
```
...
```{r}
pHRandTestResults50 <- t.test(pHRandTest50, alternative = "less", mu = nrow(diffMethStatsTreatment50)) #Conduct a one-sample t-test to determine the probability of identifying more DML by chance
...
```{r}
DML5xdiff25 <- mutate(diffMethStatsTreatment25, start = start, end = end) %>%
...
```{r}
write_delim(DML5xdiff25, "DML-5x-diff25.bed",  delim = '\t', col_names = FALSE) #Save data as a BED file
```

```{r}
DML5xdiff50 <- mutate(diffMethStatsTreatment50, start = start, end = end) %>%
  select(chr, start, end, meth.diff) %>% 
  mutate_if(is.numeric, as.integer) #Save as a BED file, and avoid writing information in scientific notation
```

```{r}
write_delim(DML5xdiff50, "DML-5x-diff50.bed",  delim = '\t', col_names = FALSE) #Save data as a BED file
```
