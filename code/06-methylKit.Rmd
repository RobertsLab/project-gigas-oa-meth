...
```{r}
differentialMethylationStatsTreatmentInd <- methylKit::calculateDiffMeth(methylationInformationFilteredCov5Ind, overdispersion = "MN", test = "Chisq") #Calculate differential methylation statistics.
head(differentialMethylationStatsTreatment) #Look at differential methylation statistics
```
...
```{r}
sampleMetadataInd <- data.frame("sampleID" = c("1", "3", "4",
                                               "6", "7", "8"),
                             "slide-label" = c("02-T1", "06-T1", "08-T2",
                                               "UK-02", "UK-04", "UK-06"),
                             "pHTreatment" = c(rep(1, times = 3),
...
for (i in 1:1000) { #For each iteration
  ...
  pHRandDML <- methylKit::reorganize(methylationInformationFilteredCov5Ind,
                                     ...
                                     treatment = pHRandTreat) %>%
    methylKit::calculateDiffMeth(., covariates = covariateMetadataInd, 
                                 overdispersion = "MN", 
                                 test = "Chisq") %>%
    methylKit::getMethylDiff(., difference = 25, qvalue = 0.01) %>%
...
hist(pHRandTest25Ind, plot = TRUE, main = "", xlab = "Number of Indale-DML (25% difference)") #Plot a histogram with randomization trial results
...
hist(pHRandTest50Ind, plot = TRUE, main = "", xlab = "Number of Indale-DML (50% difference)") #Plot a histogram with randomization trial results
