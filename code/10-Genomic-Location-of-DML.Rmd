...
cpgFeatureOverlapsAll #Confirm import
```
...
for(i in 1:ncol(cpgLocationStatTestAll)) { #For each genome feature
  GenomeFeature <- cpgLocationStatTestAll[2,i] #Variable for # genome feature overlaps for genomic CpGs
  AllFeature <- cpgLocationStatTestAll[1,i] #Variable for # genome feature overlaps for all DML
  GenomeNotFeature <- sum(cpgLocationStatTestAll[2,-i]) #Variable for # other CpG types for genomic CpGs
  ...
  rownames(ct) <- c(as.character(row.names(cpgLocationStatTestAll)[c(2,1)])) #Assign row names: genomic CpGs, all DML
...
barplot(t(t(cpgFeatureOverlapsPercentsAll[,c(2,1)])), 
        col= plotColors2, 
        axes = FALSE, 
        names.arg = c("5x CpGs", "DML"), cex.names = 1.5,
...
```{r}
geneDMLOverlaps <- read.table("DML-pH-50-Cov5-All-Gene-wb.bed", header = FALSE, sep = "\t") #Import table of gene-DML overlaps
head(geneDMLOverlaps) #Confirm import
```

```{r}
geneID <- read.table("geneID-DML-overlap.tab") #Import column with gene ID information already separated
nrow(geneID) == nrow(geneDMLOverlaps) #Check that both dataframes have the same number of rows
head(geneID) #Confim import
```

```{r}
geneDMLOverlaps <- cbind(geneDMLOverlaps[,-c(5:7,10:13)], geneID) #Add gene ID column to overlap information
colnames(geneDMLOverlaps) <- c("chr", "DMLstart", "DMLend", "meth.diff",
                               "geneStart", "geneEnd", "geneID") #Add column names
head(geneDMLOverlaps) #Confirm formatting
```

```{r}
geneDMLCounts <- as.data.frame(table(geneDMLOverlaps$geneID)) #Create a dataframe with the number of DML/gene
colnames(geneDMLCounts) <- c("geneID", "numDML") #Add column names
head(geneDMLCounts) #Confirm formatting
```

```{r}
range(geneDMLCounts$numDML) #Range of number DML/gene
hist(geneDMLCounts$numDML, ylim = c(0,20)) #Plot the frequency of multiple DML in genes. Most genes have between 1-10 DML, with one gene having > 10 and another having 104
```


```{r}
write.csv(geneDMLCounts, "Number-of-DML-per-Gene.csv", quote = FALSE, row.names = FALSE) #Save file with number of DML per gene
```
