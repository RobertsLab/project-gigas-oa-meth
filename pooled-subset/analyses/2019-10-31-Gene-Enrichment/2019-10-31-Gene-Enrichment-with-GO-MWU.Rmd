...
# Initial Biological Process information

I obtained biological process GOSlim terms for all genes and DML-Gene overlaps. I'm going to try plotting them in different ways to identify some trends in genes with DML.
...
```{r}
BPGOSlimCounts <- as.data.frame(table(BPGOSlim$GOSlim)) #Count the frequency of each GOslim term
colnames(BPGOSlimCounts) <- c("GOSlim", "frequency") #Rename columns
head(BPGOSlimCounts) #Confirm counting
```

```{r}
BPGOSlimCounts$percentage <- (BPGOSlimCounts$frequency/sum(BPGOSlimCounts$frequency)*100) #Create a new column with percentages of GOSlim terms instead of raw counts
head(BPGOSlimCounts) #Confirm column creation
```

```{r}
BPGOSlimCounts <- BPGOSlimCounts[c(4, 1, 5, 11, 13,
                                   2, 6, 3,
                                   12,
...
head(BPGOSlimCounts) #Confirm organization
```

```{r}
BPGOSlimCounts$GOSlim <- c("Cell-Cell Signaling", "Cell Adhesion", "Death", "Signal Transduction", "Transport",
...
head(BPGOSlimCounts) #Confirm capitalization
```

### DML-Gene overlaps

```{r}
DMLBPGOSlim <- read.delim("../2019-09-15-DML-Analysis/Blastquery-GOslim-DMLonly.BP.sorted.unique.noOther", sep = "\t", header = FALSE) #Import unique BP GOslim terms without any "other biological processes" entries. File is tab-delimited and does not have a header.
colnames(DMLBPGOSlim) <- c("geneID", "GOSlim") #Rename columns
head(DMLBPGOSlim) #Confirm import
```

```{r}
DMLBPGOSlimCounts <- as.data.frame(table(DMLBPGOSlim$GOSlim)) #Count the frequency of each GOslim term
colnames(DMLBPGOSlimCounts) <- c("GOSlim", "frequency") #Rename columns
head(DMLBPGOSlimCounts) #Confirm counting
```

```{r}
DMLBPGOSlimCounts$percentage <- (DMLBPGOSlimCounts$frequency/sum(DMLBPGOSlimCounts$frequency)*100) #Create a new column with percentages of GOSlim terms instead of raw counts
head(DMLBPGOSlimCounts) #Confirm column creation
```

```{r}
DMLBPGOSlimCounts <- DMLBPGOSlimCounts[c(4, 1, 5, 11, 13,
                                         2, 6, 3,
                                         12,
                                         7, 10, 9, 8),] #Reorganize rows based on broader functions. Rows are general activity, development, stress response, and metabolic processes
head(DMLBPGOSlimCounts) #Confirm organization
```

```{r}
DMLBPGOSlimCounts$GOSlim <- c("Cell-Cell Signaling", "Cell Adhesion", "Death", "Signal Transduction", "Transport",
                              "Cell Cycle and Proliferation", "Developmental Processes", "Cell Organization and Biogenesis",
                              "Stress Response",
                              "DNA Metabolism", "RNA Metabolism", "Protein Metabolism", "Other Metabolic Processes") #Capitalize labels
head(DMLBPGOSlimCounts) #Confirm capitalization
```

## Pie chart

### Reformat data for pie chart

```{r}
pieBPGOSlimCounts <- c(sum(BPGOSlimCounts$percentage[1:5]),
                       sum(BPGOSlimCounts$percentage[6:8]),
                       BPGOSlimCounts$percentage[9],
                       sum(BPGOSlimCounts$percentage[10:13])) #Add corresponding values for each category and save as a vector. Rows are General Activity (cell adhesion, cell-cell signaling, death, signal transduction, transport), Development (developmental processes, cell cycle and proliferation, cell organization and biogenesis), Stress Response (stress response), and Metabolic Processes (DNA metabolism, protein metabolism, RNA metabolism, other metabolic processes)
head(pieBPGOSlimCounts) #Confirm dataframe creation
```

```{r}
pie(pieBPGOSlimCounts, cex = 1.5,
    col = c(plotColors[2], plotColors[4], plotColors[5], plotColors[1]),
    labels = c("General Activity", "Development", "Stress Response", "Metabolic Processes")) #Create pie chart using values from pieBPGOSlimCounts. Colors are assigned based on larger groupings in labels.
```
...
```{r}
#pdf("2019-10-31-BP-GOSlim-All-Genes.pdf", width = 11, height = 8.5) #Save as a pdf

par(mar = c(5, 20, 1, 1)) #Change figure boundaries

barsGOSlim <- barplot(BPGOSlimCounts$percentage,
                      horiz = TRUE,
                      axes = FALSE,
                      xlim = c(0,14),
...
axis(side = 2, at = barsGOSlim, labels = BPGOSlimCounts$GOSlim, tick = FALSE, las = 2, col = "grey80", cex.axis = 1.5) #Add y-axis with GOSlim terms from BPGOSlimCounts. Place labels at specific barplot values from barsGOSlim. Remove tick marks (tick) and change orientation of labels (las)

axis(side = 1, at = seq(from = 0, to = 14, by = 2), cex = 1.2, col = "grey80") #Add x-axis
...
```{r}
#pdf("2019-10-31-BP-GOSlim-DML-Gene-Overlaps.pdf", width = 11, height = 8.5) #Save as a pdf

par(mar = c(5, 20, 1, 1)) #Change figure boundaries

DMLbarsGOSlim <- barplot(DMLBPGOSlimCounts$percentage,
                         horiz = TRUE,
                         axes = FALSE,
                         xlim = c(0,24),
                         col = c(rep(plotColors[2], times = 5),
...
axis(side = 2, at = DMLbarsGOSlim, labels = DMLBPGOSlimCounts$GOSlim, tick = FALSE, las = 2, col = "grey80", cex.axis = 1.5) #Add y-axis with GOSlim terms from BPGOSlimCounts. Place labels at specific barplot values from barsGOSlim. Remove tick marks (tick) and change orientation of labels (las)

axis(side = 1, at = seq(from = 0, to = 24, by = 2), cex = 1.2, col = "grey80") #Add x-axis
mtext(side = 1, "Percent of Genes with DML", line = 3, cex = 1.5) #Add x-axis label
...
```{r}
#pdf("2020-02-13-BP-GOSlim-AllGenes-Versus-DML-Recolored.pdf", width = 11, height = 8.5) #Save as a pdf

par(mfrow = c(1, 2), oma = c(5, 20, 0, 3), mar = c(0, 0, 0, 0)) #Set up parameters for multipanel plot

barsGOSlim2 <- barplot(-1*(BPGOSlimCounts$percentage),
                      horiz = TRUE,
                      axes = FALSE,
                      xlim = c(-24,0),
                      col = plotColors2[4]) #Create a barplot that is horizontal (horiz = TRUE). Use axes = FALSE to remove all axes. Set xlim based on maximum percentage value. Colors correspond to broader GOSlim groupings. Save plot as a new object.

axis(side = 2, at = barsGOSlim2, labels = BPGOSlimCounts$GOSlim, tick = FALSE, las = 2, col = "grey80", cex.axis = 1.5) #Add y-axis with GOSlim terms from BPGOSlimCounts. Place labels at specific barplot values from barsGOSlim. Remove tick marks (tick) and change orientation of labels (las)

axis(side = 1, at = seq(from = -24, to = 0, by = 2), labels = c(seq(from = 24, to = 0, by = -2)), cex = 1.2, col = "grey80") #Add x-axis
mtext(side = 1, "% Genes", line = 3, cex = 1.5) #Add x-axis label

DMLbarsGOSlim2 <- barplot(DMLBPGOSlimCounts$percentage,
                          horiz = TRUE,
                          axes = FALSE,
                          xlim = c(0,24),
                          col = plotColors2[2]) #Create a barplot that is horizontal (horiz = TRUE). Use axes = FALSE to remove all axes. Set xlim based on maximum percentage value. Colors correspond to broader GOSlim groupings. Save plot as a new object.
...
### *C. virginica* vs. *C. gigas* plot

#### Calculate relative proportions

```{r}
CgCvGOSlimCounts <- read.csv("2020-02-15-Cg-Cv-BP-GOSlim-Counts.csv", header = TRUE) #Import Cg and Cv GOSlim counts for full genomes and DML
head(CgCvGOSlimCounts) #Check file
```

```{r}
#Create a new column with percentages of GOSlim terms instead of raw counts

CgCvGOSlimCounts$CgAllpercentage <- (CgCvGOSlimCounts$CgAllfrequency/sum(CgCvGOSlimCounts$CgAllfrequency)*100) #All Cg genes

CgCvGOSlimCounts$CgDMLpercentage <- (CgCvGOSlimCounts$CgDMLfrequency/sum(CgCvGOSlimCounts$CgDMLfrequency)*100) #Cg DML

CgCvGOSlimCounts$CvAllpercentage <- (CgCvGOSlimCounts$CvAllfrequency/sum(CgCvGOSlimCounts$CvAllfrequency)*100) #All Cv genes

CgCvGOSlimCounts$CvDMLpercentage <- (CgCvGOSlimCounts$CvDMLfrequency/sum(CgCvGOSlimCounts$CvDMLfrequency)*100) #Cv DML

head(CgCvGOSlimCounts) #Confirm column creation
```

```{r}
#Create a new column with relative proportions of DML vs genes

CgCvGOSlimCounts$CgRelProp <- (CgCvGOSlimCounts$CgDMLpercentage/CgCvGOSlimCounts$CgAllpercentage) #Cg

CgCvGOSlimCounts$CvRelProp <- (CgCvGOSlimCounts$CvDMLpercentage/CgCvGOSlimCounts$CvAllpercentage) #Cg

head(CgCvGOSlimCounts) #Confirm column creation
```

```{r}
#pdf("2020-02-15-Cg-Cv-BP-GOSlim-Relative-Proportions.pdf", width = 11, height = 8.5) #Save as a pdf

par(mfrow = c(1, 2), oma = c(5, 20, 0, 3), mar = c(0, 0, 0, 0)) #Set up parameters for multipanel plot

CgbarsGOSlim <- barplot(-1*(CgCvGOSlimCounts$CvRelProp),
                      horiz = TRUE,
                      axes = FALSE,
                      xlim = c(-3,0),
                      col = plotColors2[4]) #Create a barplot that is horizontal (horiz = TRUE). Use axes = FALSE to remove all axes. Set xlim based on maximum percentage value. Colors correspond to broader GOSlim groupings. Save plot as a new object.

axis(side = 2, at = CvbarsGOSlim, labels = CgCvGOSlimCounts$GOSlim, tick = FALSE, las = 2, col = "grey80", cex.axis = 1.5) #Add y-axis with GOSlim terms from BPGOSlimCounts. Place labels at specific barplot values from barsGOSlim. Remove tick marks (tick) and change orientation of labels (las)

axis(side = 1, at = seq(from = -3, to = 0, by = 0.5), labels = c(seq(from = 3, to = 0, by = -0.5)), cex = 1.2, col = "grey80") #Add x-axis
mtext(side = 1, expression(italic("C. virginica")), line = 3, cex = 1.5) #Add x-axis label

CgbarsGOSlim <- barplot(CgCvGOSlimCounts$CgRelProp,
                          horiz = TRUE,
                          axes = FALSE,
                          xlim = c(0,3),
                          col = plotColors2[2]) #Create a barplot that is horizontal (horiz = TRUE). Use axes = FALSE to remove all axes. Set xlim based on maximum percentage value. Colors correspond to broader GOSlim groupings. Save plot as a new object.

axis(side = 1, at = seq(from = 0, to = 3, by = 0.5), cex = 1.2, col = "grey80") #Add x-axis
mtext(side = 1, expression(italic("C. gigas")), line = 3, cex = 1.5) #Add x-axis label

#dev.off()
```

# **CODE BELOW THIS HAS NOT BEEN UPDATED TO REFLECT LATEST DATASET. DO NOT USE.**

## Match gene products to GOSlim categories

```{r}
DMLGeneAnnotations <- read.delim("../2019-09-15-DML-Analysis/DML-Gene-annot.tab", sep = "\t", header = FALSE) #Import gene annotation information
DMLGeneAnnotations <- DMLGeneAnnotations[,c(1:3, 6, 10:14)] #Only keep the indicated columns
colnames(DMLGeneAnnotations) <- c("Uniprot", "geneID", "evalue", "Product", "V10", "V11", "V12", "V13", "V14") #Add column names
head(DMLGeneAnnotations) #Confirm import
```

```{r}
BPGOSlimDMLGeneAnnotations <- unique(merge(x = DMLGeneAnnotations, y = BPGOSlim, by = "geneID")) #Merge annotations with GOSlim terms by gene ID
head(BPGOSlimDMLGeneAnnotations) #Confirm merge
```

```{r}
write.csv(BPGOSlimDMLGeneAnnotations, "2019-10-31-BP-GOSlim-DML-Gene-Annotations.csv", quote = FALSE, row.names = FALSE) #Save file
```
...
```{r}
geneBackground <- read.csv("../2019-09-15-DML-Analysis/2019-09-15") #Import gene background with Uniprot annotations
...
```{r}
CVGOslim <- read.delim("../2019-08-14-Differentially-Methylated-Genes/Blastquery-GOslim-BP.sorted", sep = "\t", header = FALSE) #Import GOslim information
...
```{r}
sum(is.na(parentGOBPDMLGOslim$GOslim)) #Count how many entries do not have GOslim terms. 356/6111 do not have GO-MWU entries.
```
...
parentGOBPDMLGOslimCounts <- subset(parentGOBPDMLGOslimCounts, subset = frequency &gt; 0) #Remove residual factors
