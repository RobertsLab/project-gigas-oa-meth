...
```{r}
barplot(shortMIBPGOCounts$frequency)
barplot(longMIBPGOCounts$frequency)
```

The two histograms look very similar, but I'll go ahead and look at percents just to be safe.
...
```{r}
barplot(shortMIBPGOtermCounts$frequency)
barplot(longMIBPGOtermCounts$frequency)
```
